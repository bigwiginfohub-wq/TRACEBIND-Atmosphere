"""TRACEBIND Publication-Grade Validation & Statistical Diagnostics Suite (v2 - Fixed)"""
...
def run_stage_1_data_generation(
    ...
    with open(csv_path, mode="w", newline="") as f:
        ...
        for run_idx, (l_val, j_val, m_val, reg_val, rep) in enumerate(grid, 1):
            seed = 30000 + run_idx
            rng = np.random.default_rng(seed)
            ...
            if j_val > 0.0:
                nx, ny = grid_shape
                xx, yy = np.meshgrid(np.arange(nx), np.arange(ny), indexing="ij")
                dx_warp = rng.normal(0.0, j_val, size=grid_shape)
                dy_warp = rng.normal(0.0, j_val, size=grid_shape)
...
# ==============================================================================
# STAGE 2: TRUE FACTORIAL ANOVA VIA STATSMODELS
# ==============================================================================
...
def run_stage_2_factorial_anova(csv_path: str = "factorial_results.csv"):
    print("\n" + "=" * 120)
    print(
        "      STAGE 2: FULL 3-WAY FACTORIAL ANOVA WITH INTERACTION SPACE (RESPONSE: ΔR)"
    )
    print("=" * 120)

    df = pd.read_csv(csv_path, comment="#")

    model_formula = "delta_R ~ C(corr_length) * C(jitter) * C(mask_ratio)"
    ols_model = ols(model_formula, data=df).fit()
    anova_table = anova_lm(ols_model, typ=2)

    # Explicitly calculate Mean Square column missing in statsmodels output
...
# ==============================================================================
# STAGE 3: REGRESSION & COMPREHENSIVE DIAGNOSTIC SUITE
# ==============================================================================
...
def run_stage_3_mechanistic_regression(
    ...
    print("\n" + "=" * 120)
    print("      STAGE 3: MECHANISTIC REGRESSION & ADVANCED STATISTICAL DIAGNOSTICS")
    print("=" * 120)
    ...
    t_stats = ols_res.tvalues[1:]
    partial_r2_exact = (t_stats**2) / (t_stats**2 + ols_res.df_resid)
    ...
    print("-" * 105)
    print(
        f"{'Predictor':<18} | {'Std Beta (β)':<14} | {'Std Error':<10} |"
        f" {'t-stat':<10} | {'p-value':<10} | {'VIF':<8} | {'Tolerance':<10} |"
        f" {'Partial R²':<10}"
    )
    print("-" * 105)
    print(
        f"{'Intercept':<18} | {ols_res.params[0]:<+14.4f} |"
        f" {ols_res.bse[0]:<10.4f} | {ols_res.tvalues[0]:<+10.4f} |"
        f" {ols_res.pvalues[0]:<10.2e} | {'N/A':<8} | {'N/A':<10} | {'N/A':<10}"
    )

    for idx, name in enumerate(X_cols):
        p_str = (
            f"{ols_res.pvalues[idx+1]:<10.2e}"
            if ols_res.pvalues[idx+1] >= 1e-100
            else "< 1.00e-100"
        )
        print(
            f"{name:<18} | {ols_res.params[idx+1]:<+14.4f} |"
            f" {ols_res.bse[idx+1]:<10.4f} | {ols_res.tvalues[idx+1]:<+10.4f} |"
            f" {p_str} | {vifs[idx]:<8.2f} | {tolerances[idx]:<10.4f} |"
            f" {partial_r2_exact[idx]:<10.4f}"
        )

    print("-" * 105)
    print(
        f"Full Model R²: {ols_res.rsquared:.4f} | Adjusted R²:"
        f" {ols_res.rsquared_adj:.4f}"
    )

    bp_test = het_breuschpagan(ols_res.resid, X_sm)
    dw_stat = durbin_watson(ols_res.resid)
    ...
    print(f"Breusch-Pagan Homoscedasticity Test LM p-value : {bp_test[1]:.4e} {bp_msg}")
    print(
        f"Durbin-Watson Autocorrelation Statistic      : {dw_stat:.4f} (Ideal ~ 2.0)"
    )
    cook_msg = (
    ...
    print(f"Maximum Cook's Distance (Outlier Influence)   : {max_cooks_d:.4f} {cook_msg}")
    print("." * 60)
    ...
    ax.set_title(
        "TRACEBIND Sensitivity: ΔR vs. ΔMoran's I with Exact Statistical Envelopes",
        fontsize=12,
        fontweight="bold",
    )
    ...
    plt.close()
    print(
        f"\n[INFO] Diagnostic Plot saved with Mean & Prediction Bands to: {output_plot.resolve()}"
    )
    print("=" * 120 + "\n")
